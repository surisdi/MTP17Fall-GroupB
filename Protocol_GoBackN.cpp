...
void GoBackN::receiverQueueThread(std::atomic<bool>* get_out, std::queue<char*> &q){
...
void GoBackN::receiverProcessingThread(std::atomic<bool>* get_out, std::queue<char*> &q){
    //Start the timer
	startTime = std::millis();
	...
	// Stop the timer 
	stopTime = std::millis();
	numBytes = utils::PAYLOAD_L_GBN*counter;
	float rate = numBytes / (stopTime - startTime);
	COUT << "Transfer complete at "<<rate<<" KB/s \n\r";
    if (ret) {
    	COUT << "Error writing the file";
    } else {
    	COUT << "Success - End of processing thread";
    }
}
