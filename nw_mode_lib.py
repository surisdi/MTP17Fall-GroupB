...
NEXT = TEAM_C  # TX for next time slot
TX_POS = [0,0,0,0]
RX_POS = [0,0,0,0]
TX_ACK = [0,0,0,0]
RX_ACK = [0,0,0,0]
...
def init_comms():
    ...
    radio_Tx.setChannel(RF_CH[0])
    radio_Rx.setChannel(RF_CH[1])
...
def network_main():
    ...
    SEND_CTRL = not radio_Rx.available(0)
    ...
    while (TX_CMPLT < 3 and RX_CMPLT < 3 and time.time() < (start_time + TMAX)):
        if SEND_CTRL:
            ...
            if (received_acks()):
                ...
                print ("Received less than 2 ACKS")
...
class PKT:
    def __init__(self, typ=0, header=0x00, payload=""):
        self.typ = typ
        self.header = header
            ...
            if (self.header < 128):
                ...
                if (WAITING_DATA):
                    ...
                    # Doing nothing here, TX and NEXT modified outside (min ACKs is 2)
                        print ("ACK to my team")

                    else:
                        if (SEND_CTRL):
                        # Discard, someone is trying to win our channel
                            print ("Someone is trying to win our channel")
                        else:
    ...
    def is_ACK(self):
        return (self.typ == 0 and (self.header >> 5) == TX)
    ...
    # Output: True or False.
    def is_my_data(self):
        return (self.is_data() and (self.header >> 5) == MY_TEAM)
    ...
    def is_expected_data(self):
        return (self.is_data() and (self.header & 31) == RX_POS[TX] + 1)
...
def generate_data(text_file, index=0):
    ...
    data = text_in_bin[
           index * len_packet: index * len_packet + len_packet - 1]  # a partition of length len_packet of the text_file is taken
...
def send_data():
    ...
    packet = PKT()
    if TX_POS[0] < POS_MAX:
        with open("text_file_A.txt", "rb") as f:
            payload = generate_data(index=TX_POS[0], text_file=f)
            packet.generate_pkt(1, payload, 0)
            packet.send_pkt()

    # Sending to team B
    if TX_POS[1] < POS_MAX:
        with open("text_file_C.txt", "rb") as f:
            payload = generate_data(index=TX_POS[2], text_file=f)
            packet.generate_pkt(1, payload, 1)
            packet.send_pkt()

    # Sending to team D
    if TX_POS[3] < POS_MAX:
        with open("text_file_D.txt", "rb") as f:
            payload = generate_data(index=TX_POS[3], text_file=f)
            packet.generate_pkt(1, payload, 1)
            packet.send_pkt()
...
def store_data(tx, payload):
    ...
    with open(file_str, file_mode) as f:
