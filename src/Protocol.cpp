...
{
	char *packet = (char *)malloc(sizeof(char) * (comms::code_length));
	std::string corrected = "";
	corrected.resize(comms::data_length, 0x00);
	...
	while (true) {
		...
		n = socket->read_blocking(packet, comms::code_length);
        ...
        spacket.assign(packet, comms::code_length);
        data = spacket.substr(0, comms::data_length);
...
int StopWait::send_text(char *text) {
    ...
    while (i < len / PAYLOAD + extra) {
        ...
        message.resize(comms::code_length, 0x00); //fill with 0x00 if the data is not long enough to form the packet
        ...
        utils::bsc(packet, comms::code_length, 0.0);
        double w = (double)rand()/ (double) RAND_MAX;
        if (w>0.5){
        	socket->write_socket(packet, comms::code_length);
        }
